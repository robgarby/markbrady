...
if (($data['scriptName'] ?? '') === 'processPharmacyMeds') {
    ...
    // 1) check DIN exists
    $sqlCheckDin = "SELECT ID FROM medications_2026 WHERE DIN = ? LIMIT 1";
    ...
    // 2) insert (ONLY name + DIN)
    // NOTE: if your table has other NOT NULL cols with no defaults, this will fail until you add defaults or include them here.
    $sqlInsert = "INSERT INTO medications_2026 (medication, DIN) VALUES (?, ?)";
    ...
    foreach ($medications as $m) {
        ...
        $medName = trim((string) ($m['medication'] ?? ''));
        ...
        $stmtInsert->bind_param("ss", $medName, $din);
